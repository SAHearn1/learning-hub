...
import { generateSummativeAssessment } from '@/lib/assessments/summative-generator';
...
export const POST = withApiHandler(async (req: NextRequest) => {
  ...
  if (!hasRequiredMinorConsent(user.isMinor, user.consentStatus)) {
    throw new ForbiddenError('Parental consent required before summative assessments');
  }
  ...
  if (!studentId || !sessionId || !topicName || !learningObjectives) {
    throw new BadRequestError(
      'Missing required fields: studentId, sessionId, topicName, learningObjectives'
    );
  }
...
export const GET = withApiHandler(async (req: NextRequest) => {
  ...
  if (!studentId && !sessionId && !topicId) {
    throw new BadRequestError('studentId, sessionId, or topicId is required');
  }
