...
| Prod auth runtime | Clerk live keys missing/mismatched in Vercel production | Set `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=pk_live_...` and `CLERK_SECRET_KEY=sk_live_...` in Vercel **Production**, then redeploy.             |
...
## Vercel production auth env parity (follow-up to #174)

After production migrations are confirmed green, verify Vercel production auth keys:
...
2. Update the two Clerk variables to live keys.
3. Redeploy production and re-check sign-in flows.
